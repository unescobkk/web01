...
function condate2Num ($date){
	...
	if(strpos($date, " ")){ // ไม่มีการเปลี่ยนวัน
			...
			return $tmp_date['2']."-".$thai_month_arr[$tmp_date['1']]."-".$tmp_date['0'];
	}else{
		$tmp_date =  explode('/', $date);
...
function gen_inv_no ($code,$sub_code){
 ...
 $po_date = condate2Num($_POST['input_date']);
 $dv_date = condate2Num($_POST['input_date1']);
...
// $_FILES['userImage']['name'];
   $sql = "UPDATE  inventory SET 
	...
	if($_FILES["userImage"]["name"] != ""){
			...
			echo $sql_pic = "UPDATE picture SET picurl = '{$_FILES['userImage']['name']}' WHERE FormID = '{$_POST['id']}' ";
