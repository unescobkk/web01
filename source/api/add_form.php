	...
	$(document).ready(function (e){
		...
		e.preventDefault();
			$.ajax({
				...
				alert(data);
				location.reload();parent.jQuery.fancybox.close()
					//parent.jQuery.fancybox.close()
	...
	$(document).ready(function(){              
		 $('#cost').change(function() {
			...
			$.ajax({
				...
				cache	: false,
				url		: "add_thai2us.php",
				data	: {cost:this.value,datepicker1:$( "#datepicker1" ).val()},
				success: function(data) {
	 ...
	 $(document).ready(function(){              
		 $('#cost_us').change(function() {
			...
			$.ajax({
				...
				cache	: false,
				url		: "add_us2thai.php",
				data	: {cost_us:this.value,datepicker1:$( "#datepicker1" ).val()},
				success: function(data) {
				//	alert(data);
					$("#cost").val(data);
				}
     ...
     function GetChar (event){
          ...
          if( !(event.keyCode == 8                                // backspace
				...
				|| event.keyCode == 110                              // delete
				|| event.keyCode == 190                              // delete
				|| (event.keyCode >= 35 && event.keyCode <= 40)     // arrow keys/home/end
				|| (event.keyCode >= 46 && event.keyCode <= 57)     // numbers on keyboard
...
<div id='preview'></div>
<form action="add_item.php" method="post" class="basic-grey"  id = "update" name = "update" enctype="multipart/form-data" >
	...
	<label>
        <span>Room No :</span>
		<input id="room" type="text" name="room"  value = ""  onkeydown = "GetChar(event);" />
    </label> 
	<p>
     <label>
        <span>PO.No. :</span>
		  <input id="po_no" type="text" name="po_no" value = "" onkeydown = "GetChar(event);"/>
    </label> 
	<label>
        <span>PO.Date :</span>
	...
	<label>
        <span>Cost(THB)  :</span>
		<input id="cost" type="text" name="cost"  value = "" onkeydown = "GetChar(event);" />
	
    </label> 
	
	<label>
        <span>Cost(USD):</span>
		<input id="cost_us" type="text" name="cost_us" value = ""  onkeydown = "GetChar(event);"/>
	...
	<p>
     <label>
        <span>Budget Cost :</span>
		  <input id="budget_code" type="text" name="budget_code"  value = "" />
    </label> 
	<label>
        <span>Bus. Area :</span>
		<input id="busa" type="text" name="busa"  value = ""/>
    </label>
	<p>
     <label>
        <span>Status. :</span>
	...
	<span  style = "padding-left:35px;">CPU Speed :</span>
      <input id="name" type="text" name="cpuspeed" id="cpuspeed" class = "tb2"  value = "" onkeydown = "GetChar(event);" />MHz.
		
    </label>
    <p>
    <label>
        <span>RAM :</span>
         <input id="name" type="text" name="ram" id = "ram" class = "tb2"   value = "" onkeydown = "GetChar(event);" />MB.
       
    </label>
   
    <label>
        <span  style = "padding-left:80px;">Harddisk :</span>
        <input id="name" type="text" name="hd" id = "hd" class = "tb2" value = "" onkeydown = "GetChar(event);"  />GB.
    </label> 
	<p>
     <label>
